...
@Component
public class TransactionCreatedDeadLetterPublisher {
    private static final Logger log = LoggerFactory.getLogger(TransactionCreatedDeadLetterPublisher.class);
    private static final String FAILED_SERVICE = "risk-engine-service";
    ...
    public TransactionCreatedDeadLetterPublisher(
    ...
    public void publish(ConsumerRecord<?, ?> record, Exception exception, int attempt) {
        ...
        try {
            kafkaTemplate.send(KafkaTopics.TRANSACTION_CREATED_DLQ, originalKey(record), deadLetterEvent)
            ...
            kafkaEventLogRepository.save(new KafkaEventLogEntity(
                    ...
                    record.topic(),
                    RiskEngineProcessor.CONSUMER_GROUP,
            ...
            log.warn(
                    ...
                    record.topic(),
                    originalKey(record),
                    KafkaTopics.TRANSACTION_CREATED_DLQ,
                    RiskEngineProcessor.CONSUMER_GROUP,
                    deadLetterEvent.error().errorType(),
                    deadLetterEvent.error().errorMessage()
            );
        } catch (Exception sendException) {
            throw new IllegalStateException("Failed to publish transaction.created DLQ event", sendException);
        }
    }
    ...
    private static String errorType(Throwable throwable) {
        ...
        if (className.contains("kafka") || className.contains("redis") || className.contains("timeout")) {
            return "DEPENDENCY_UNAVAILABLE";
        }
